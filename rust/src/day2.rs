...
fn play<T>(input: &str, parse: fn (&str) -> T, get_outcome: fn (Shape, T) -> (Shape, Outcome)) -> i32 {
    Shape::parse(input, parse)
        .into_iter()
        .fold(0, |acc, (a, b)| {
            let (shape, outcome) = get_outcome(a, b);
            acc
                + match shape { Shape::Rock => 1, Shape::Paper => 2, Shape::Scissors => 3, }
                + match outcome { Outcome::Win => 6, Outcome::Draw => 3, Outcome::Lose => 0, }
        })
}

fn part1(input: &str) -> i32 {
    play(input, Shape::parse_shape_letter, |a, b| {
        match (a, b) {
...
fn part2(input: &str) -> i32 {
    ...
    play(input, parse_outcome_letter, |a, b| {
        match (a, b) {
            (a, Outcome::Draw) => (a, b),
